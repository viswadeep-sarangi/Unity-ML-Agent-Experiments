...
public class HumanoidWalkerAgent : Agent
{
    ...
    void FixedUpdate()
    {
        ...
        /** These are additional constraints put in by me (Viswadeep Sarangi) with the hopes of getting closer to real walkins
         * 1. Left/Right Leg (knee and foot) velocities' forward component have to be similar in value but opposite in sign. Thus, penalize the absolute value of their sum
         * 2. Left/Right Arm (upper and lower) velocities' forward component have to be similar in value but opposite in sign
         */
        AddReward(
            -0.025f* Mathf.Abs(Vector3.Dot(m_JdController.bodyPartsDict[hips].rb.velocity, m_JdController.bodyPartsDict[handL].rb.velocity + m_JdController.bodyPartsDict[handR].rb.velocity)) // hand velocities
            // TODO
        );
    }
